...
public abstract class PircBot implements ReplyConstants
{
    ...
    public final synchronized void connect(final String hostname) throws IOException, IrcException, NickAlreadyInUseException {
        this.connect(hostname, 6667, null);
    }
    
    public final synchronized void connect(final String hostname, final int port) throws IOException, IrcException, NickAlreadyInUseException {
        this.connect(hostname, port, null);
    }
    
    public final synchronized void connect(final String hostname, final int port, final String password) throws IOException, IrcException, NickAlreadyInUseException {
        ...
        while ((line = breader.readLine()) != null) {
            ...
            if (secondSpace >= 0) {
                ...
                if (code.equals("433")) {
                    ...
                    ++tries;
                    nick = String.valueOf(this.getName()) + tries;
                    OutputThread.sendRawLine(this, bwriter, "NICK " + nick);
                }
    ...
    public final synchronized void sendRawLineViaQueue(final String line) {
        ...
        if (this.isConnected()) {
            this._outQueue.add((Object)line);
        }
    }
    
    public final void sendMessage(final String target, final String message) {
        this._outQueue.add((Object)("PRIVMSG " + target + " :" + message));
    }
    ...
    public final void sendNotice(final String target, final String notice) {
        this._outQueue.add((Object)("NOTICE " + target + " :" + notice));
    }
    
    public final void sendCTCPCommand(final String target, final String command) {
        this._outQueue.add((Object)("PRIVMSG " + target + " :\u0001" + command + "\u0001"));
    }
    ...
    protected void handleLine(final String line) {
        ...
        if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
            ...
            if (request.equals("VERSION")) {
                ...
                final boolean success = this._dccManager.processRequest(sourceNick, sourceLogin, sourceHostname, request);
                if (!success) {
                    this.onUnknown(line);
                }
            }
    ...
    private final void processServerResponse(final int code, final String response) {
        if (code == 322) {
            ...
            final String topic3 = this._topics.get(channel3);
            this._topics.remove(channel3);
            this.onTopic(channel3, topic3, setBy, date, false);
        }
        else if (code == 353) {
            final int channelEndIndex = response.indexOf(" :");
            final String channel3 = response.substring(response.lastIndexOf(32, channelEndIndex - 1) + 1, channelEndIndex);
    ...
    private final void processMode(final String target, final String sourceNick, final String sourceLogin, final String sourceHostname, final String mode) {
    ...
    protected void onSetChannelKey(final String channel, final String sourceNick, final String sourceLogin, final String sourceHostname, final String key) {
    }
    
    protected void onRemoveChannelKey(final String channel, final String sourceNick, final String sourceLogin, final String sourceHostname, final String key) {
    }
    
    protected void onSetChannelLimit(final String channel, final String sourceNick, final String sourceLogin, final String sourceHostname, final int limit) {
    }
    ...
    protected void onSetChannelBan(final String channel, final String sourceNick, final String sourceLogin, final String sourceHostname, final String hostmask) {
    }
    
    protected void onRemoveChannelBan(final String channel, final String sourceNick, final String sourceLogin, final String sourceHostname, final String hostmask) {
    }
    ...
    @Deprecated
    protected void onDccSendRequest(final String sourceNick, final String sourceLogin, final String sourceHostname, final String filename, final long address, final int port, final int size) {
    }
    
    @Deprecated
    protected void onDccChatRequest(final String sourceNick, final String sourceLogin, final String sourceHostname, final long address, final int port) {
    }
    ...
    private final void addUser(String channel, final User user) {
        channel = channel.toLowerCase();
        synchronized (this._channels) {
            Hashtable users = this._channels.get(channel);
            if (users == null) {
                users = new Hashtable();
